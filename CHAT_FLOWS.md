...
| Unsupported request                | `Send Taylor the notes`                                                 | `Sorry, I can only help with scheduling events.`                                  |
...
| Cal.com timeout/network            | Any Cal.com-backed request during timeout/network failure               | `Cal.com timed out or could not be reached. Please try again.`                    |
...
| Invalid confirmation reply  | `maybe`    | `Say yes to confirm or no to cancel.` |

### Booking Error Cases

| Case                                        | User input/state                                                       | Expected response example                                                      |
| ------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| No available slots                          | Request a time outside availability                                    | `No available slots in that window. What other time works for you?`            |
...
| Slot becomes unavailable after confirmation | Choose slot, then `yes`, while Cal.com rejects it                      | `That slot is no longer available. Please choose another time.`                |
...
| Timeout while confirming                    | Confirm booking during timeout                                         | `Cal.com timed out or could not be reached. Please try again.`                 |
| Unexpected response while confirming        | Confirm booking when Cal.com returns malformed data                    | `Cal.com returned an unexpected response. Please try again.`                   |
...
| Invalid confirmation reply       | After cancel confirmation, type `maybe`      | `Say yes to cancel this booking, or no to keep it.`                                     |
| No matching booking              | `Cancel my meeting with Unknown LiveX Guest` | `I couldn't find a matching booking.`                                                   |
| Already cancelled booking        | Cancel an event that is already cancelled    | `That booking is already cancelled.`                                                    |
...
| Already cancelled booking        | Reschedule an already cancelled event                            | `That booking is already cancelled and can't be rescheduled.`                                                                         |
...
| Invalid multiple-match selection | After multiple matches, type `banana`                            | Repeats the matches and adds: `Please reply with a number.`                                                                           |
| No available slots               | `Move my Taylor call to tomorrow at 3am`                         | `No available slots in that window. What other time works for you?`                                                                   |
...
| Cancel word at confirmation | `cancel`                       | `Got it, no changes made.`                                     |
| Invalid confirmation reply  | `maybe`                        | `Say yes to reschedule, or no to keep the original time.`      |
